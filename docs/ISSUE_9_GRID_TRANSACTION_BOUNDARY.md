...
The visual-profile editing control violated the `IDataGridViewEditingControl` contract by writing directly to `DataGridView.CurrentCell.Value`. That raised `CellValueChanged` inside the selector's own input stack.

`ConfigurationForm` then committed settings. The settings notification rebuilt the same grid with `Rows.Clear()` while the cell was still committing, causing the original exception.
...
- `ModernVisualProfileEditingControl` reports a dirty value through `EditingControlValueChanged` and `NotifyCurrentCellDirty(true)`. It does not mutate a grid cell directly and does not force `EndEdit`.
